	...
	while True:
		while not q.empty():
			...
			try:
				...
				print("/VehicleBackend/imageDir/"+ imageDirectory+"/"+filename, date, timestamp)
				status = ocr.dbPush(plateNumber, "/VehicleBackend/imageDir/"+ imageDirectory+"/"+filename, date, timestamp)
				if status == False:
					# with open("imageLinks.txt", "a", encoding='utf-8') as f:
...
#			(lib.network_width(netMain), lib.network_height(netMain)))
	...
	print(lib.network_width(netMain),lib.network_height(netMain))
